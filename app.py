...
def about_menu_ui():
    ...
    with st.expander("实用网站导航", expanded=False):
        st.markdown("优质网络、公开工具、学习网站、娱乐站点等网站的收录，仅作导航作用")
        st.markdown("## ⚠ 免责声明")
        st.markdown("上述网站收录本页面仅为第三方网站导航收录平台。除特殊标注说明外，所有收录网站均为**公开网络资源**，并非作者创建、运营。用户自主访问、使用外部网站所产生的一切行为与后果，由用户自行承担，请严格遵守国家相关法律法规与各平台用户协议。")
